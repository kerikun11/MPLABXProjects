...
void send_timer_interrupt(void) {
    if (cnt_clk < 32) {
        ...
        LAT = 0;
    } else if ((cnt_clk - 36) < 32) {
        LAT = 0;
        if ((cnt_clk - 36) % 2 == 0) {
            CLK = 1;
            DAT = (serial_data2 >> (15 - (cnt_clk - 36) / 2));
        } else {
            CLK = 0;
        }
    } else if ((cnt_clk - 36) == 33) {
        CLK = 1;
    } else if ((cnt_clk - 36) == 34) {
        LAT = 1;
    } else if ((cnt_clk - 36) == 35) {
        LAT = 0;
    }
    if ((cnt_clk - 36) < 35)cnt_clk++;
}

void data_set(uint8_t value) {
    serial_data1 = (value << 9) + 0x00;
    serial_data2 = (value << 9) + 0x10;
    cnt_clk = 0;
}
...
int main(void) {
    ...
    while (1) {
        ...
        if (sw_p.flag.long_holding_1 == 1) {
            if (vol == 0x7F)vol = 95;
            else if (vol >= LONG_HOLD_STEP) vol -= LONG_HOLD_STEP;
            else vol = 0;
            data_set(vol);
            __delay_ms(50);
        }
        ...
        if (sw_m.flag.long_holding_1 == 1) {
            if (vol <= 95 - LONG_HOLD_STEP) vol += LONG_HOLD_STEP;
            else vol = 0x7F;
            data_set(vol);
            __delay_ms(50);
        }
