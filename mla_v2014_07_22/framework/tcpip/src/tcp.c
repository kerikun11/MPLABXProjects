...
// should always be enabled on PIC24 products.  On PIC32
// products there is very little difference and depnds on compiler
// optimization level
#if defined(__XC16) && !defined(TCP_OPTIMIZE_FOR_SIZE)
    #define TCP_OPTIMIZE_FOR_SIZE
#elif defined(__XC32) && defined(TCP_OPTIMIZE_FOR_SIZE)
...
static uint16_t NextPort __attribute__((persistent));   // Tracking variable for next local client port number
#endif
...
static void TCPRAMCopy(PTR_BASE wDest, uint8_t vDestType, PTR_BASE wSource, uint8_t vSourceType, uint16_t wLength);
...
void TCPInit(void)
{
    ...
    for(i = 0; i < TCP_SOCKET_COUNT; i++)
    {
        ...
        switch(vMedium)
        {
            ...
            default:
                while(1); // Undefined allocation medium.  Go fix your tcpip_config.h TCP memory allocations.
        }
...
/*****************************************************************************
  ...
  Input:
    ...
    vSocketPurpose -   Any of the TCP_PURPOSE_* constants defined in
                       tcpip_config.h or the TCPIPConfig utility (see
...
/*****************************************************************************
  ...
  Returns:
    ...
    time TCPGetRemoteInfo() is called.
        0 if the user specifies an invalid socket number.
  ***************************************************************************/
SOCKET_INFO* TCPGetRemoteInfo(TCP_SOCKET hTCP)
{
...
/*****************************************************************************
  ...
  ***************************************************************************/
static void TCPRAMCopy(PTR_BASE ptrDest, uint8_t vDestType, PTR_BASE ptrSource, uint8_t vSourceType, uint16_t wLength)
{
        ...
        case TCP_PIC_RAM:
            switch(vDestType)
            {
                ...
                case TCP_ETH_RAM:
                    ...
                    MACPutArray((uint8_t*)ptrSource, wLength);
                    break;
        ...
        case TCP_ETH_RAM:
            switch(vDestType)
            {
                case TCP_PIC_RAM:
                    ...
                    MACGetArray((uint8_t*)ptrDest, wLength);
                    break;
