...
#define __UDP_PERFORMANCE_TEST_C_
...
void UDPPerformanceTask(void)
{
    ...
    #if defined(STACK_USE_DHCP_CLIENT) && defined(STACK_USE_UDP_PERFORMANCE_TEST)
    {
    ...
    MySocket = UDPOpenEx((uint32_t)(PTR_BASE)&Remote,UDP_OPEN_NODE_INFO,0,PERFORMANCE_PORT);
    ...
    UDPPutArray((uint8_t*)&dwCounter, sizeof(dwCounter));
    dwCounter++;

    wTemp = UDPPutROMArray((ROM uint8_t*)
