/*******************************************************************************
  ...
  Description:
    Berkeley Socket Distribution(BSD) APIs for Microchip TCPIP Stack
...
int sendto( SOCKET s, const char* buf, int len, int flags, const struct sockaddr* to, int tolen )
{
    ...
    static uint32_t startTick;      // NOTE: startTick really should be a per socket BSDSocket structure member since other BSD calls can interfere with the ARP cycles
    ...
    if(socket->SocketType == SOCK_DGRAM) //UDP
    {
        ...
        if(UDPIsPutReady(socket->SocketID))
        {
            ...
            size = UDPPutArray((uint8_t*)buf, len);
            UDPFlush();
            return size;
        }
    }
...
int recv( SOCKET s, char* buf, int len, int flags )
{
    ...
    if(socket->SocketType == SOCK_STREAM) //TCP
    {
        ...
        return TCPGetArray(socket->SocketID, (uint8_t*)buf, len);
    }
    else if(socket->SocketType == SOCK_DGRAM) //UDP
    {
        ...
        if(UDPIsGetReady(socket->SocketID))
            return UDPGetArray((uint8_t*)buf, len);
    }
