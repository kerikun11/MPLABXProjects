...
/****************************************************************************
  ...
  ***************************************************************************/
    ...
    static uint8_t sslStubID;           // Which SSL_STUB is loaded
    static SSL_KEYS sslKeys;        // The current SSL session
    static uint8_t sslKeysID;           // Which SSL_KEYS are loaded
    static uint8_t sslBufferID;     // Which buffer is loaded
    static uint8_t sslHashID;           // Which hash is loaded
    static uint8_t sslSessionID;        // Which session is loaded
    static bool sslSessionUpdated;  // Whether or not it has been updated
    static uint8_t sslRSAStubID;        // Which stub is using RSA, if any
    ...
    uint8_t *ptrHS;                 // Used in buffering handshake results

    extern ROM uint16_t SSL_CERT_LEN;   // RSA public certificate length        ?
    extern ROM uint8_t SSL_CERT[];      // RSA public certificate data          ?
...
/****************************************************************************
  ...
  ***************************************************************************/
    ...
    static void SSLStubSync(uint8_t id);
    ...
    static void SaveOffChip(uint8_t *ramAddr, PTR_BASE ethAddr, uint16_t len);
    static void LoadOffChip(uint8_t *ramAddr, PTR_BASE ethAddr, uint16_t len);
    ...
    static uint16_t HSPutROMArray(TCP_SOCKET skt, ROM uint8_t *data, uint16_t len);
    #endif
...
/****************************************************************************
  ...
  ***************************************************************************/
uint16_t isStubUsed;            // Indicates which stubs are in use
uint16_t isHashUsed;            // Indicates which hashes are in use
uint16_t isBufferUsed;          // Indicates which buffers are in use

// Masks for each bit in the is*Used variables
static ROM uint16_t masks[16] = { 0x0001, 0x0002, 0x0004, 0x0008,
...
uint16_t SSLRxRecord(TCP_SOCKET hTCP, uint8_t id)
{
    ...
    if(sslStub.wRxBytesRem == 0u)
    {
        ...
        if((sslStub.rxProtocol & 0x80) == 0x80)
        {
            ...
            ((uint8_t*)&sslStub.wRxBytesRem)[1] = sslStub.rxProtocol & 0x7F;
            TCPGet(hTCP, ((uint8_t*)&sslStub.wRxBytesRem));
        ...
        #endif
        {
            ...
            TCPGet(hTCP, ((uint8_t*)&sslStub.wRxBytesRem)+1);
            TCPGet(hTCP, ((uint8_t*)&sslStub.wRxBytesRem));
...
static void SSLRxServerCertificate(TCP_SOCKET hTCP)
{
    ...
    switch(sslStub.dwTemp.v[0]) {
        ...
        case RX_SERVER_CERT_START:
            ...
            len = TCPFindROMArray(hTCP, (ROM uint8_t*)"\x2A\x86\x48\x86\xF7\x0D\x01\x01\x01", 9, 0, false);
...
static void SSLTxClientKeyExchange(TCP_SOCKET hTCP)
{
    ...
    TCPPutArray(hTCP, (uint8_t*) sslBuffer.full + len, len);
...
void GenerateSessionKeys(void)
{
    ...
    if(sslStub.Flags.bIsServer)
    {
        ...
        ARCFOURInitialize(&(sslKeys.Remote.app.cryptCtx), (uint8_t*)(&sslHash), 16);
        SSLBufferSync(sslStub.idTxBuffer);
        sslKeys.Local.app.cryptCtx.Sbox = sslBuffer.full;
        ARCFOURInitialize(&(sslKeys.Local.app.cryptCtx), (uint8_t*)(&sslHash)+16, 16);
    ...
    SSLBufferSync(sslStub.idTxBuffer);
    sslKeys.Local.app.cryptCtx.Sbox = sslBuffer.full;
    ARCFOURInitialize(&(sslKeys.Local.app.cryptCtx), (uint8_t*)(&sslHash), 16);
    SSLBufferSync(sslStub.idRxBuffer);
    sslKeys.Remote.app.cryptCtx.Sbox = sslBuffer.full;
    ARCFOURInitialize(&(sslKeys.Remote.app.cryptCtx), (uint8_t*)(&sslHash)+16, 16);
    #endif
...
static void CalculateFinishedHash(uint8_t hashID, bool fromClient, uint8_t *result)
{
    ...
    if(fromClient)
        HashAddROMData(&sslHash, (ROM uint8_t*)"CLNT", 4);
    else
        HashAddROMData(&sslHash, (ROM uint8_t*)"SRVR", 4);
    ...
    if(sslHash.hashType == HASH_SHA1)
        i--;
    for(; i > 0u; i--)
        HashAddROMData(&sslHash, (ROM uint8_t*)"\x36\x36\x36\x36\x36\x36\x36\x36", 8);
    ...
    if(sslHash.hashType == HASH_SHA1)
        i--;
    for(; i > 0u; i--)
        HashAddROMData(&sslHash, (ROM uint8_t*)"\x5c\x5c\x5c\x5c\x5c\x5c\x5c\x5c", 8);
...
/*********************************************************************
 * Function:        static void SSLStubSync(uint8_t id)
 *
 * PreCondition:    None
 *
...
static void SSLKeysSync(uint8_t id)
{
    ...
    if(sslKeysID != SSL_INVALID_ID)
        SaveOffChip((uint8_t*)&sslKeys,
            SSL_BASE_KEYS_ADDR+SSL_KEYS_SIZE*sslKeysID,
            SSL_KEYS_SIZE);

    // Load new stub
    LoadOffChip((uint8_t*)&sslKeys,
...
static uint8_t SSLSessionNew(void)
{
    ...
    if(id < MAX_SSL_SESSIONS)
    {
        ...
        if(sslSessionUpdated)
            SaveOffChip((uint8_t*)&sslSession,
...
static void SSLSessionSync(uint8_t id)
{
    ...
    // Load new buffer
    LoadOffChip((uint8_t*)&sslSession,
...
/*********************************************************************
 * Function:        static void SaveOffChip(uint8_t *ramAddr,
...
/*********************************************************************
 * Function:        static void LoadOffChip(uint8_t *ramAddr,
 *                                          uint16_t ethAddr, uint16_t len)
 *
 * PreCondition:    None
 *
...
//  while(w)
//  {
//      uint16_t wChunkLen = sizeof(sslBuffer.full) - (ptrHS - sslBuffer.full);
//      if(wChunkLen == 0u)
//      {
//          HSEnd();
//          HSStart();
...
void SSLMACBegin(uint8_t *MACSecret, uint32_t seq, uint8_t protocol, uint16_t len)
{
    ...
    HashAddROMData(&sslHash, (ROM uint8_t*)"\0\0\0\0", 4);
