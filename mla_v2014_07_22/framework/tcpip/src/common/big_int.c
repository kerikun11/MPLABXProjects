...
extern void _masBIROM(void);
    ...
    void putulhexUART(uint32_t dw)
    {
        ...
        putcUART('x');
        while(BusyUART());
        putcUART(btohexa_high(((uint8_t*)&dw)[3]));
        while(BusyUART());
        putcUART(btohexa_low(((uint8_t*)&dw)[3]));
        while(BusyUART());
        putcUART(btohexa_high(((uint8_t*)&dw)[2]));
        while(BusyUART());
        putcUART(btohexa_low(((uint8_t*)&dw)[2]));
        while(BusyUART());
        putcUART(btohexa_high(((uint8_t*)&dw)[1]));
        while(BusyUART());
        putcUART(btohexa_low(((uint8_t*)&dw)[1]));
        while(BusyUART());
        putcUART(btohexa_high(((uint8_t*)&dw)[0]));
        while(BusyUART());
        putcUART(btohexa_low(((uint8_t*)&dw)[0]));
    }
#endif
 ...
 *                  wWordLength: the number of words in the integer (a word is 1 byte on PIC18s, 2 bytes on PIC24)
 *
...
#if defined(__XC8) && defined(BI_USE_CONSTRUCTOR_ROM)
...
void BigIntMod(BIGINT *n, BIGINT* m)
{
    ...
    while(_iR >= n->ptrLSB)
    {
        ...
        // by a power of 2^8 (PIC18) or 2^16 (PIC24) or 2^32 (PIC32)
...
#if defined(__XC8) && defined(BI_USE_MOD_ROM)
...
#if defined(__XC8) && defined(BI_USE_COMPARE_ROM)
int8_t BigIntCompareROM(BIGINT *a, BIGINT_ROM *b)
{
    PTR_BASE                magA, magB;
    BIGINT_DATA_TYPE        valA, valB;
    BIGINT_DATA_TYPE        *ptrA;
...
#if defined(__XC8) && defined(BI_USE_MAG_DIFF_ROM)
...
#if defined(__XC8) && defined(BI_USE_MAG_ROM)
 ...
 *                  multiplication in base 256 (PIC18) or 65536 (PIC24).
...
#if defined(__XC8) && defined(BI_USE_MULTIPLY_ROM)
...
#if defined(__XC8) && defined(BI_USE_ADD_ROM)
...
#if defined(__XC8) && defined(BI_USE_SUBTRACT_ROM)
// Note: This function is not required by either RSA operation
...
#if defined(__XC8) && defined(BI_USE_COPY_ROM)
