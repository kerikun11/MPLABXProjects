...
/****************************************************************************
  ...
  ***************************************************************************/
        ...
        static void HTTPLoadConn(uint8_t hHTTP);
    #endif
    HTTP_STUB httpStubs[MAX_HTTP_CONNECTIONS];  // HTTP stubs with state machine and socket
    uint8_t curHTTPID;                              // ID of the currently loaded HTTP_CONN

/****************************************************************************
  Section:
    Function Prototypes
  ***************************************************************************/
    static void HTTPHeaderParseLookup(uint8_t i);
    ...
    static HTTP_READ_STATUS HTTPReadTo(uint8_t delim, uint8_t* buf, uint16_t len);
    #endif
...
void HTTPInit(void)
{
    for(curHTTPID = 0; curHTTPID < MAX_HTTP_CONNECTIONS; curHTTPID++)
    {
        ...
        #if !defined(HTTP_SAVE_CONTEXT_IN_PIC_RAM)
        {
            ...
            MACPutArray((uint8_t*)&curHTTP, sizeof(HTTP_CONN));
...
static void HTTPLoadConn(uint8_t hHTTP)
{
    ...
    MACGetArray((uint8_t*)&curHTTP, sizeof(HTTP_CONN));
    ...
    do
    {
        ...
        case SM_HTTP_PARSE_REQUEST:
            ...
            {// First line isn't here yet
                ...
                if((int32_t)(TickGet() - curHTTP.callbackID) > (int32_t)0)
        ...
        case SM_HTTP_SERVE_HEADERS:
            ...
            // Send headers
            TCPPutROMString(sktHTTP, (ROM uint8_t*)HTTPResponseHeaders[curHTTP.httpStatus]);
            ...
            if(curHTTP.httpStatus == HTTP_REDIRECT)
            {
                ...
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"\r\n\r\n304 Redirect: ");
                TCPPutString(sktHTTP, curHTTP.data);
                TCPPutROMString(sktHTTP, (ROM uint8_t*)HTTP_CRLF);
            }
            ...
            {
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"Content-Type: ");
                TCPPutROMString(sktHTTP, (ROM uint8_t*)httpContentTypes[curHTTP.fileType]);
            ...
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"Cache-Control: ");
            if(curHTTP.httpStatus == HTTP_POST || curHTTP.nextCallback != 0xffffffff)
            {// This is a dynamic page or a POST request, so no cache
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"no-cache");
            }
            else
            {// This is a static page, so save it for the specified amount of time
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"max-age=");
                TCPPutROMString(sktHTTP, (ROM uint8_t*)HTTP_CACHE_LEN);
            }
            TCPPutROMString(sktHTTP, HTTP_CRLF);
        ...
        case SM_HTTP_SERVE_COOKIES:
            ...
            {
                // Write the header
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"Set-Cookie: ");
...
static void HTTPHeaderParseCookie(void)
{
    ...
    if(lenB >= (uint16_t)(curHTTP.data + HTTP_MAX_DATA_LEN - curHTTP.ptrData - 2))
    {// If not, overflow
        curHTTP.httpStatus = HTTP_OVERFLOW;
        smHTTP = SM_HTTP_SERVE_HEADERS;
        return;
...
/*****************************************************************************
  ...
  Description:
    ...
    conversions are made: to 曾0 to 曾0 to  and
    ...
    for(pRead = pWrite = cData; wLen != 0u; )
    {
        ...
        else if(c == '%')
        {
            ...
            else
            {
                ...
                wLen--;
                wLen--;
                *pWrite++ = hexatob(*((TCPIP_UINT16_VAL*)&hex));
            }
        }
        else
            *pWrite++ = c;
...
/*****************************************************************************
  Function:
    static HTTP_READ_STATUS HTTPReadTo(uint8_t cDelim, uint8_t* cData, uint16_t wLen)
...
void HTTPIncFile(ROM uint8_t* cFile)
{
    ...
    {// On initial call, open the file and save its ID
        ...
        ((TCPIP_UINT32_VAL*)&curHTTP.callbackPos)->w[0] = MPFSGetID(fp);
    }
    else
    {// The file was already opened, so load up its ID and seek
        fp = MPFSOpenID(((TCPIP_UINT32_VAL*)&curHTTP.callbackPos)->w[0]);
        if(fp == MPFS_INVALID_HANDLE)
        {// No file handles available, so wait for now
            return;
        }
        MPFSSeek(fp, ((TCPIP_UINT32_VAL*)&curHTTP.callbackPos)->w[1], MPFS_SEEK_FORWARD);
    }
    ...
    ((TCPIP_UINT32_VAL*)&curHTTP.callbackPos)->w[1] = MPFSTell(fp);
    MPFSClose(fp);
