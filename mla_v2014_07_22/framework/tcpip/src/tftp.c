...
static uint32_t _tftpStartTick;
...
static void _TFTPSendAck(TCPIP_UINT16_VAL blockNumber);
    ...
    static void _TFTPSendROMFileName(TFTP_OPCODE opcode, ROM uint8_t *fileName);
#endif
...
static ROM uint8_t *vUploadRemoteHost;
static ROM uint8_t *vUploadFilename;
...
/*****************************************************************************
  Function:
    void TFTPUploadRAMFileToHost(ROM uint8_t *vRemoteHost,
                                 ROM uint8_t *vFilename,
                                 uint8_t *vData,
  ...
  Parameters:
    vRemoteHost: ROM string of the remote TFTP server to upload to (ex:
        ...
        between RAM and ROM pointers (PIC24 and PIC32), this string must
  ...
  Remarks:
    ...
    STACK_USE_DNS must be defined in tcpip_config.h.
...
/*****************************************************************************
  Function:
    void TFTPUploadFragmentedRAMFileToHost(ROM uint8_t *vRemoteHost,
                                           ROM uint8_t *vFilename,
...
int8_t TFTPGetUploadStatus(void)
{
    ...
    if(UDPIsOpened(_tftpSocket)== false)
    {
        ...
        _tftpSocket = UDPOpenEx((uint32_t)(ROM_PTR_BASE)vUploadRemoteHost,
...
void TFTPOpenFile(uint8_t *fileName, TFTP_FILE_MODE mode)
{
    ...
    memcpy(&UDPSocketInfo[_tftpSocket].remote.remoteHost, &MutExVar.group1._hostInfo.IPAddr.Val, sizeof(uint32_t));
...
/*********************************************************************
 * Function:        void TFTPPut(uint8_t c)
 *
