...
#define __TCP_PERFORMANCE_TEST_C_
...
/*****************************************************************************
  ...
  Description:
    ...
    tcpip_config.h.  Round-trip time is affected by the distance to the
...
void TCPTXPerformanceTask(void)
{
    ...
    static uint32_t dwTimeStart;
    static uint32_t dwBytesSent;
    static TCPIP_UINT32_VAL dwVLine;
    uint8_t vBuffer[10];
    static uint8_t vBytesPerSecond[12];
    uint16_t w;
    uint32_t dw;
    uint64_t qw;
    ...
    while(w >= 12+27+5+32u)
    {
        ...
        TCPPutROMString(MySocket, (ROM uint8_t*)": We are currently achieving ");
        TCPPutROMArray(MySocket, (ROM uint8_t*)"       ", 5-strlen((char*)vBytesPerSecond));
        TCPPutString(MySocket, vBytesPerSecond);
        TCPPutROMString(MySocket, (ROM uint8_t*)"00 bytes/second TX throughput.\r\n");
    ...
    {
//      qw = ((uint64_t)dwBytesSent) * (TICK_SECOND/100/8);
//      qw = ((uint64_t)dwBytesSent) * (TICK_SECOND/100);
//      qw /= dw;
        qw = ((uint64_t)dwBytesSent)*(TICK_SECOND/100) + (dw>>1);
        qw /= dw;
        ultoa((uint32_t)qw, vBytesPerSecond);
        dwTimeStart += dw;
...
void TCPRXPerformanceTask(void)
{
    ...
    if(dw > TICK_SECOND)
    {
        ...
        qw = (uint64_t)dwBytesRead * TICK_SECOND;
        qw += dw>>1;
        qw /= dw;
        ultoa((uint32_t)qw, vBuffer);
        TCPPutString(MySocket, vBuffer);
        TCPPutROMString(MySocket, (ROM uint8_t*)" bytes/second\r\n");
