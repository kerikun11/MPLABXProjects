...
typedef struct WFMacStatsStruct
{
    ...
    /**
      ...
      one and the WEPOn value for the key mapped to the transmitter痴 MAC address indicates the frame
      should not have been encrypted.
      */
    uint32_t MibRxWEPUndecryptCtr;
    uint32_t MibRxFragAgedCtr; // Number of times that fragments 疎ged out or were not received in the allowable time.
...
{
    /**
      SSID, which must be less than or equal to 32 characters.  Set to all 0痴
    ...
    /**
      ...
      Default: BSSID not used (all FF痴)
      */
    ...
    /**
      ...
      <table>
        WEP Keys        If using WEP this field must contain 4 keys.  Each key must be
                         ...
                         index 15.  Unused keys should be set to all 0痴.
...
typedef struct WFCAElementsStruct
{
    ...
    /**
      ...
      2.  An SSID is defined in the Connection Profile and multiple AP痴 are discovered with the same SSID.
    ...
    uint8_t   rssi;
    /**
      <b>Note: Connection Profile lists are not yet supported.  This array should be set to all FF痴.</b>
      */
...
typedef struct
{
    ...
    typedef struct
    {
        ...
        uint8_t keyLen;     /* WPA/WPA2-PSK key length */
...
/*------------------------*/
    ...
    typedef enum
    {
        ...
        WF_MODULE_WF_TX_POWER                = 14,  /* drv_wifi_tx_power.c              */
        WF_MODULE_WF_POWER_SAVE              = 15,  /* drv_wifi_power_save.c            */
        WF_MODULE_EVENT_HANDLER              = 16,  /* drv_wifi_event_handler.c         */
        WF_MODULE_WF_SCAN                    = 17,  /* drv_wifi_scan.c                 */
        WF_MODULE_DEBUG_STRINGS              = 18,  /* drv_wifi_debug_strings.c */
        WF_MODULE_IPERF_APP                  = 19, /* iperf_app.c               */
...
/*******************************************************************************
  ...
  Parameters:
    p_cpIdList - Pointer to value representing the bit mask where each bit
                  ...
                  Connection Profile ID闢 1 and and 2 have been created.
...
/*******************************************************************************
  ...
  Parameters:
    CpId - If this value is equal to an existing Connection Profile鐃 ID than
...
/*******************************************************************************
  Function:
  WFEnableDebugPrint(uint8_t option)
...
/*******************************************************************************
  ...
  Description:
    ...
    up they are OR壇 together the MRF24WB0M will receive and pass to the Host
...
/*******************************************************************************
  ...
  Description:
    ...
    up they are OR壇 together the MRF24W will receive and pass to the Host
    CPU received packets from either multicast address.
    ...
    * macBytes -- Array containing the MAC address to filter on (using the destination
                ...
                MAC address can be designated as 租on稚 carebytes.  See macBitMask.
    ...
    * macBitMask -- A byte where bits 5:0 correspond to macBytes[5:0].  If the bit is
                  ...
                  corresponding MAC byte is a 租on稚 careand not used in the
  ...
  Parameters:
    CpId - Connection Profile to use.
            ...
            * If CP has a defined SSID only scan results with that SSID are
               retained.
            * If CP does not have a defined SSID then all scanned SSID痴 will be
               retained
            ...
            * All scan results are retained (both Infrastructure and Ad Hoc
               networks).
            * All channels within the MRF24W痴 regional domain will be
               scanned.
...
#ifdef WIFI_NET_TEST
    extern void wifi_net_test_print(char *str, uint32_t param);
#endif
