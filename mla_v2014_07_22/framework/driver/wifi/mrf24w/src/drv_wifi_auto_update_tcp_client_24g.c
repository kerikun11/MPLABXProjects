/*******************************************************************************
  ...
  Description:
    WiFi module FW auto update viva TCP connection
...
// This is the example for 0x3107 FW version
//
...
static uint8_t PatchName[]     = "/mrfupdates/A2Patch_test_3160.txt";
//static uint8_t PatchName[]     = "/mrfupdates/A2Patch_test_3180.txt";
...
static void parse_byte(uint8_t c)
{
    ...
    case 3:
        ...
        break;
    case 4:
        parse_tmp[0]=0; parse_tmp[1]=0;parse_tmp[2]=0;parse_tmp[3]=0;
    default:
        break;
...
static void parse_checksum(void)
{
    ...
    //for(i=0;i<24;i++){ SYS_CONSOLE_PRINT("%02x;",buf_checksum[i]);} SYS_CONSOLE_MESSAGE("\r\n");
...
void AutoUpdate_TCPClient(void)
{
    ...
    switch(AutoUpdateTCPExampleState)
    {
        case SM_UPDATE_HOME:
            ...
            for(i=0;i<12;i++) buf_checksum[i]=0;
            buf_checksum_index = 0;
            for(i=0;i<4;i++) parse_tmp[i] = 0;
            parse_index = 0;
            SizeOfReceive = 0;
        ...
        case SM_UPDATE_READ_DATA:
            ...
            w = TCPIsGetReady(MySocket);
            j = sizeof(vBuffer)-1;
                        if(w!=0){ char buf_t[30]; putsUART("-!\r\n!-Total="); sprintf(buf_t, "%u,Current=%ud, size=%u: ",SizeOfPackage, SizeOfReceive,w);putsUART(buf_t);}
            while(w)
            {
                int i;
                if(w < j){j = w; }
