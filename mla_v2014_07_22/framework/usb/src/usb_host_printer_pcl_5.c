...
typedef struct _PRINTER_STATUS_PCL
{
    ...
    uint8_t    imageEndMask;   // Some PCL printers do not ignore bits after the specified
...
/****************************************************************************
  Function:
    uint8_t USBHostPrinterLanguagePCL5( uint8_t address,
        ...
        USB_PRINTER_COMMAND command, USB_DATA_POINTER data, uint32_t size, uint8_t transferFlags )
{
    ...
    switch( command )
    {
        ...
        case USB_PRINTER_FONT_NAME:
            ...
            printerListPCL[printer].fontName = (uint8_t)size;
        ...
        case USB_PRINTER_FONT_SIZE:
            ...
            printerListPCL[printer].fontSize = (uint8_t)size;
            return _PrintFontCommand( printer, transferFlags );
            break;
        ...
        case USB_PRINTER_SET_POSITION:
            ...
            sprintf( buffer, COMMAND_POSITION_HORIZONTAL COMMAND_POSITION_VERTICAL, (uint16_t)(size >> 16) * 10, (uint16_t)size * 10 );
        ...
        case USB_PRINTER_IMAGE_DATA_HEADER:
            ...
            sprintf( buffer, COMMAND_RASTER_COMPRESSION_NONE COMMAND_RASTER_DATA, (uint16_t)((size+7)/8) );
            USBHOSTPRINTER_SETFLAG_COPY_DATA( transferFlags );
            return USBHostPrinterWrite( printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags );
            break;
        ...
        case USB_PRINTER_GRAPHICS_MOVE_TO:
            ...
            sprintf( buffer, COMMAND_GRAPHICS_MOVE_TO, (uint16_t)(size >> 16) * 10, (uint16_t)size * 10);
            _SetCurrentPosition( printer, (uint16_t)(size >> 16), (uint16_t)size );
            USBHOSTPRINTER_SETFLAG_COPY_DATA( transferFlags );
            return USBHostPrinterWrite( printerListPCL[printer].deviceAddress, buffer, strlen(buffer), transferFlags );
            break;
