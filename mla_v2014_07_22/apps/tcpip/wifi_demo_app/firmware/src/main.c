    ...
    while(1)
    {
        ...
        if(dwLastIP != AppConfig.MyIPAddr.Val)
        {
            ...
            if(AppConfig.Flags.bIsDHCPEnabled && DHCPIsBound(0)) {
                static uint8_t  tftpInitDone = 0;
                static uint8_t dummy_file[] = "TFTP test dummy contents";
                static ROM uint8_t file_name[] = "dontcare";
                static ROM uint8_t host_name[] = "tftp" WIFI_NET_TEST_DOMAIN;
                if (!tftpInitDone) {
        ...
        {
            static uint32_t t_UpdateImage=0;
            extern uint8_t Flag_ImageUpdate_running;
...
static void ProcessIO(void)
{
    ...
    uitoa(*((uint16_t*)(&ADRESL)), AN0String);
#endif
}
