...
static HTTP_IO_RESULT HTTPPostWifiConfig(void)
{
   ...
   // WF_CMGetConnectionState(&ConnectionState, &ConnectionProfileID);
    ...
    while(curHTTP.byteCount)
    {
        ...
        else if(!strcmppgm2ram((char*)curHTTP.data, "key"))
        {
            ...
            if (!ascii_key)
            {
                //if ((cfg.security == sec_wep64) || (cfg.security == sec_wep128))
                    key_size /= 2;
                if (!convertAsciiToHexInPlace((int8_t *)&CFGCXT.key[0], key_size))
                {
        ...
        else if(!strcmppgm2ram((char*)curHTTP.data, "ssid"))
        {
            if(strlen((char*)(curHTTP.data+6)) < 33u)
            {
                ...
                // WF_CPGetSsid(ConnectionProfileID, (uint8_t*)&CFGCXT.prevSSID, &ssidLen);
    ...
    /* SoftAP: Support only OPEN and WEP */
...
void HTTPPrint_ipaddr(void)
{
    ...
    TCPPutString(sktHTTP, (uint8_t *)ipAddr);
}
...
void HTTPPrint_curPrivacy(void)
{
    ...
    switch (AppConfig.SecurityMode)
    {
        case 0:
            ...
            break;
        case 1:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WEP");
            break;
        case 2:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WEP");
            break;
        case 3:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPA");
            break;
        case 4:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPA");
            break;
        case 5:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPA2");
            break;
        case 6:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPA2");
            break;
        case 7:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPA-AUTO");
            break;
        case 8:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPA-AUTO");
            break;
        case 9 :
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPS PBC");
            break;
        case 10:
            TCPPutROMString(sktHTTP, (ROM uint8_t*)"WPS PIN");
            break;
        default:
            // impossible to get here!
            break;
...
void HTTPPrint_wlan(void)
{
    ...
    if (bssDescIsValid)
    {
        if ((IS_SCAN_STATE_DISPLAY(SCANCXT.scanState)) || (IS_SCAN_STATE_VALID(SCANCXT.scanState)))
        {
        ...
        else
        {
            if(AppConfig.networkType == WF_ADHOC)
                ...
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"WiFi Direct P2P (BSS)");
            else
                TCPPutROMString(sktHTTP, (ROM uint8_t*)"Unknown BSS");
        }
        bssDescIsValid = false;
    }
    else
...
void HTTPPrint_aplist(void)
{
    ...
    for(x=0; x < SCANCXT.numScanResults; x++)
    {
        ...
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"<bss>\n");
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"<name>");
        TCPPutString(sktHTTP, bssDesc.ssid);
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"</name>\n");
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"<privacy>");
        security = (bssDesc.apConfig & 0xd0) >> 4;
        uitoa(security, secString);
        TCPPutString(sktHTTP, secString);
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"</privacy>\n");
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"<wlan>");
        uitoa(bssDesc.bssType, bssTypeString);
        TCPPutString(sktHTTP, bssTypeString);
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"</wlan>\n");
        TCPPutROMString(sktHTTP, (ROM uint8_t*)"<strength>");
        if (bssDesc.rssi < 61)
        {
            strVal = 1;
        }
