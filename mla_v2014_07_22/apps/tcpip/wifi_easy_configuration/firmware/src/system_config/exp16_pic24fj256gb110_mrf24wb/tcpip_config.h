...
#include "iperf_app.h" // Iperf is used in Easy Configuration demo
 ...
 * check whether the files (eg generic_tcp_client.c) are located in folder (eg \apps\tcpip\wifi_easy_configuration\firmware\src).
 * You may need to copy such files from the wifi_demo_app or wifi_console folder.
 *
 */
...
//#define STACK_USE_GENERIC_TCP_CLIENT_EXAMPLE  // HTTP Client example in generic_tcp_client.c. If using EZConfig, need to copy this file from Demo App or WiFi Console.
//#define STACK_USE_GENERIC_TCP_SERVER_EXAMPLE  // ToUpper server example in generic_tcp_server.c. If using EZConfig, need to copy this file from Demo App or WiFi Console.
...
//#define STACK_USE_WF_MCAST_TEST
#if defined(STACK_USE_WF_MCAST_TEST) && defined(STACK_USE_ZEROCONF_MDNS_SD)
#error "when testing MCAST filter, please disable STACK_USE_ZEROCONF_MDNS_SD"
#endif
...
#define MY_DEFAULT_IP_ADDR_SOFTAP_BYTE1 (192ul)
#define MY_DEFAULT_IP_ADDR_SOFTAP_BYTE2 (168ul)
#define MY_DEFAULT_IP_ADDR_SOFTAP_BYTE3 (1ul)
#define MY_DEFAULT_IP_ADDR_SOFTAP_BYTE4 (3ul)
