...
void TDES_OFBEncrypt_header_example1(BLOCK_CIPHER_HANDLE handle);
void TDES_OFBDecrypt_header_example1(BLOCK_CIPHER_HANDLE handle);

void TDES_OFB_Examples(BLOCK_CIPHER_HANDLE handle)
{
    TDES_OFBEncrypt_header_example1(handle);
    TDES_OFBDecrypt_header_example1(handle);
}
...
void TDES_OFBEncrypt_header_example1(BLOCK_CIPHER_HANDLE handle)
{
    ...
    static uint8_t TDESKey[]                = { 0x37, 0x8c, 0x89, 0xd3, 0xb0, 0x91, 0x70, 0x80,
                                                    0x2a, 0xf1, 0x80, 0x13, 0xc2, 0xef, 0x62, 0x10,
                                                    0x37, 0x8c, 0x89, 0xd3, 0xb0, 0x91, 0x70, 0x80
                                                  };

    static uint8_t initial_data[]           = { 0x00, 0x78, 0x17, 0xd4, 0xbb, 0xdc, 0x69, 0xa1
                                                  };

    static uint8_t plain_text[]             = { 0x16, 0x82, 0x60, 0xd5, 0xfa, 0xed, 0x24, 0xb9,
    ...
    uint8_t keyStream[TDES_BLOCK_SIZE*4];
    BLOCK_CIPHER_STATE state;
    uint32_t numCipherBytes;

    // Initialize the Block Cipher context
    BLOCK_CIPHER_16BV1_OFB_Initialize (handle, &context, CRYPTO_16BV1_ALGORITHM_TDES, CRYPTO_16BV1_ALGORITHM_TDES, TDES_BLOCK_SIZE, (uint8_t *)initial_data, (void *)&keyStream, sizeof (keyStream), &TDESKey[0], CRYPTO_KEY_SOFTWARE, CRYPTO_64DES_3_KEY);
    ...
    //Encrypt the data
    BLOCK_CIPHER_16BV1_OFB_Encrypt (handle, cipher_text, &numCipherBytes, (void *) plain_text, sizeof(plain_text), BLOCK_CIPHER_OPTION_STREAM_CONTINUE);
...
void TDES_OFBDecrypt_header_example1(BLOCK_CIPHER_HANDLE handle)
{
    ...
    static uint8_t TDESKey[]                = { 0x8c, 0x83, 0xf7, 0x04, 0x5b, 0xc7, 0x62, 0x67,
                                                    0x70, 0xb9, 0xfd, 0x0b, 0xc8, 0x85, 0x67, 0xa7,
                                                    0x8c, 0x83, 0xf7, 0x04, 0x5b, 0xc7, 0x62, 0x67
                                                  };

    static uint8_t initial_data[]           = { 0xe4, 0x14, 0x9e, 0x9f, 0xec, 0x84, 0xf2, 0xf7
                                                  };

    static uint8_t cipher_text[]             = {
    ...
    //Encrypt the data
    BLOCK_CIPHER_16BV1_OFB_Decrypt (handle, plain_text, &numPlainBytes, (void *) cipher_text, sizeof(cipher_text), BLOCK_CIPHER_OPTION_STREAM_CONTINUE);
