...
if(SYS_CLK_FrequencySystemGet() <= 125000)
    ...
    #define CLOCK_DIVIDER_SETTING 0x0000 
    #define SYMBOL_TO_TICK_RATE 125000
if(SYS_CLK_FrequencySystemGet() <= 1000000)
...
#define SYMBOL_TO_TICK_RATE (SYS_CLK_FrequencySystemGet())


#define ONE_SECOND (((uint32_t)SYS_CLK_FrequencySystemGet()/1000 * 62500) / (SYMBOL_TO_TICK_RATE / 1000))
/* SYMBOLS_TO_TICKS to only be used with input (a) as a constant, otherwise you will blow up the code */
#define SYMBOLS_TO_TICKS(a) (((uint32_t)(SYS_CLK_FrequencySystemGet()/100000) * a) / (SYMBOL_TO_TICK_RATE / 100000))
#define TICKS_TO_SYMBOLS(a) (((uint32_t)SYMBOL_TO_TICK_RATE/100000) * a / ((uint32_t)SYS_CLK_FrequencyInstructionGet()/100000))
