...
v1.21 or later (in PRO mode), for the PIC16F145x family of USB microcontrollers.

Note: This firmware can only be used in conjunction with PIC16F145x
revision A5 or later devices (ex: cannot be used with A2 silicon).  The
A2 silicon devices have a PFM (program flash memory) self write errata that
precludes USB bootloader operation (see issue #3 from DS80000546E).
...
0x000-0x8FF (14-bit WORD addresses).  The application firmware is
supposed to occupy the 0x900-[end of flash] region of program memory.
The addresses 0x900-0x903 are part of the application address space, but
must be reserved for storing the flash signature word and application
image version words. 

In order for the bootloader firmware to fit within the 0x000-0x8FF region, this 
project must be built with the XC8 PRO mode compiler optimizations enabled.
...
1. Under the build configuration-->XC8 global options-->XC8 linker-->Option categories:Additional options
   the "Codeoffset" must be set to: 0x904
   See the "Required_Application_Project_Linker_Settings_Codeoffset.png" image for example view.

2. Under the build configuration-->XC8 global options-->XC8 linker-->Option categories:Memory model,
   the "ROM ranges" must be set to: default,-0-903
   See the "Required_Application_Project_Linker_Settings_ROM_Ranges_Exclusion.png" image for example view.
