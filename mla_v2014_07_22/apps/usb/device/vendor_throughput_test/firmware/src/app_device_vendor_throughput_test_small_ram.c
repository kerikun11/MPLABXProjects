/********************************************************************
 ...
 (the "Company") for its PIC・ス Microcontroller is intended and
...
//SRAM, while on other devices, the buffers can be placed anywhere.
    ...
    //All other variables are only accessed by the CPU and can be placed anywhere in RAM.
    #pragma udata
#endif

#if defined(__XC8) && defined(FIXED_ADDRESS_MEMORY)
    USB_VOLATILE uint8_t EP1OUTEvenBuffer[64] @ EP1_EVEN_DATA_BUFFER_ADDRESS;	//User buffer for receiving OUT packets sent from the host
    USB_VOLATILE uint8_t EP1OUTOddBuffer[64]  @ EP1_ODD_DATA_BUFFER_ADDRESS;	//User buffer for receiving OUT packets sent from the host
#else
    USB_VOLATILE uint8_t EP1OUTEvenBuffer[64];	//User buffer for receiving OUT packets sent from the host
    USB_VOLATILE uint8_t EP1OUTOddBuffer[64];	//User buffer for receiving OUT packets sent from the host
#endif
...
void APP_DeviceVendorThroughputTestInitialize()
{
    ...
    EP2OUTEvenHandle = USBTransferOnePacket(2, OUT_FROM_HOST,(uint8_t*)&EP1OUTEvenBuffer,64);	//Should not be using the EP1OUTEvenBuffer.  Should use a dedicated EP2OUTEvenBuffer instead.
    EP2OUTOddHandle = USBTransferOnePacket(2, OUT_FROM_HOST,(uint8_t*)&EP1OUTOddBuffer,64);	//Should not be using the EP1OUTOddBuffer.  Should use a dedicated EP2OUTOddBuffer instead.
    EP2OUTEvenNeedsServicingNext = true;	//Used to keep track of which buffer will contain the next sequential data packet.

    EP3OUTEvenHandle = USBTransferOnePacket(3, OUT_FROM_HOST,(uint8_t*)&EP1OUTEvenBuffer,64);	//Should not be using the EP1OUTEvenBuffer.  Should use a dedicated EP3OUTEvenBuffer instead.
    EP3OUTOddHandle = USBTransferOnePacket(3, OUT_FROM_HOST,(uint8_t*)&EP1OUTOddBuffer,64);	//Should not be using the EP1OUTOddBuffer.  Should use a dedicated EP3OUTOddBuffer instead.
