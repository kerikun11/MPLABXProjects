...
static uint16_t XCoordinate1;
static uint16_t YCoordinate1;
static uint16_t XCoordinate2;
static uint16_t YCoordinate2;
static uint16_t XCoordinate3;
static uint16_t YCoordinate3;
static uint16_t XOrigin1;
static uint16_t YOrigin1;
static uint16_t XOrigin2;
static uint16_t YOrigin2;
static uint16_t XOrigin3;
static uint16_t YOrigin3;
...
static uint8_t movement_length;
static uint8_t vector;
//The direction that the mouse will move in
static const signed char dir_table[]={-4,-4,-4, 0, 4, 4, 4, 0};
...
static void Emulate_Mouse(void)
{
    ...
    if(!USBHandleBusy(lastTransmission))
    {
        ...
        lastTransmission = HIDTxPacket(HID_EP, (uint8_t*)hid_report_in, 0x04);
...
static void Emulate_Digitizer(void)
{
    ...
    if(!USBHandleBusy(lastTransmission))
    {
        ...
        //velocity movement).
        uint16_t SOFDelta = (SOFCountIntSafe - SOFCountIntSafeOld);
        ...
        if(DeviceMode == MULTI_TOUCH_DIGITIZER_MODE)
        {
            ...
            lastTransmission = HIDTxPacket(HID_EP, (uint8_t*)hid_report_in, 20);
        }
        else //must be DeviceMode == SINGLE_TOUCH_DIGITIZER_MODE
        {
            ...
            //Send the packet now.
            lastTransmission = HIDTxPacket(HID_EP, (uint8_t*)hid_report_in, 7);
        }
...
{
    static uint8_t FeatureReport[3];
    uint8_t bytesToSend;
