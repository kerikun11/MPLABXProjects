/* 
 * File:   PIC18F27J53-ChargeStation6-09.h
 * Author: kerikun11
 *
 * Created on August 11, 2015, 9:22 AM
 */

#ifndef PIC18F27J53_CHARGESTATION6_09_H
#define	PIC18F27J53_CHARGESTATION6_09_H
...
void port_button_loop(void) {
    ...
    for (i = 0; i < NUM_PORT; i++) {
        ...
        if (port[i].ctmu.flag.press) {
            ...
            // 充電状態でなければ、充電開始表示
            if (!port[i].flag.out_flag) {
                display_now(START_CHARGING);
                display_reserve(PRINT_DATA, 32);
            } else {
...
void normal_loop(void) {
    ...
    if (delay_display.flag) {
        ...
        pres_print_item = next_print_item;
        display_flag = 1;
    }
    if (display_flag) {
        display_flag = 0;
    ...
    // Serial Data Out
    if (data_out && data_out_flag) {
        data_out_flag = 0;
        char str[15];
        print_time(&usb_tx, 0);
        for (uint8_t i = 0; i < NUM_PORT; i++) {
            ringbuf_put_str(&usb_tx, "  \tPORT:");
            switch (i) {
                case PORT_L:
                    ringbuf_put_str(&usb_tx, "L");
                    break;
                case PORT_C:
                    ringbuf_put_str(&usb_tx, "C");
                    break;
                case PORT_R:
                    ringbuf_put_str(&usb_tx, "R");
                    break;
            }
            ringbuf_put_str(&usb_tx, "\t");
            ringbuf_put(&usb_tx, port[i].current / 1000 + '0');
            ringbuf_put(&usb_tx, '.');
            my_utoa32(str, port[i].current % 1000, 3);
            ringbuf_put_str(&usb_tx, str);
            ringbuf_put_str(&usb_tx, "A");

            ringbuf_put_str(&usb_tx, "\t");
            my_utoa32(str, port[i].integrated_current, 7);
            ringbuf_put_str(&usb_tx, str);
            ringbuf_put_str(&usb_tx, "mAh");
        }
        ringbuf_put_str(&usb_tx, "\n");
    }
}
...
#endif	/* PIC18F27J53_CHARGESTATION6_09_H */
