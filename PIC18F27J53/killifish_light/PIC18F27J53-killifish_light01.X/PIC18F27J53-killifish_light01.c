...
void rx_operation(void) {
    if (rx_flag) {
        ...
        if (!strcmp(opcode, "set")) {
            if (!strcmp(opland1, "sync")) {
                light_mode = TIME_SYNC_mode;
            }
            if (!strcmp(opland1, "adc")) {
                light_mode = ADC_mode;
            }
            if (!strcmp(opland1, "r")) {
                light_mode = COMMAND_mode;
                PWM_set(0, atoi(opland2));
            }
            if (!strcmp(opland1, "g")) {
                light_mode = COMMAND_mode;
                PWM_set(1, atoi(opland2));
            }
            if (!strcmp(opland1, "b")) {
                light_mode = COMMAND_mode;
                PWM_set(2, atoi(opland2));
            }
            if (!strcmp(opland1, "w")) {
                light_mode = COMMAND_mode;
                PWM_set(3, atoi(opland2));
            }
        }
    }
}
...
void main_init(void) {
    ...
    //timer0_init(6); // óp
    timer1_init(0, T1OSC); // óp
    timer3_init(2); // buttonóp
    ADC_init(VDD);
    RTCC_init();
